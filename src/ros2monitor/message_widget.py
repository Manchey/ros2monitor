...
class MessageWidget(ScrollWidget):
    ...
    def render(self):
        ...
        if rate_text != "unknown" or last_valid_time + self._topic_timeout * 1e9 <= current_rostime:
        ...
        self._end = (
            self.render_message(self._split_sign, self._topic_name, self._topic_info.last_message)
            + 1
        )
    ...
    def expand_topic(self, topic):
        if self._show_value[topic]:
            [
                self.expand_topic(t)
                for t in self._show_value.keys()
                if self.is_parent_topic(topic, t)
            ]
        else:
    ...
    def render_message(self, parent_topic, topic_name, message, row=0, col=0):
        ...
        if hasattr(message, "get_fields_and_field_types"):
            ...
            type(message) in (list, tuple)
            and (len(message) > 0)
            and hasattr(message[0], "__slots__")
        ):
            if self._show_value[topic_full_name]:
